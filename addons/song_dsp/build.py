#!/usr/bin/env python3
"""Build the song_dsp GDExtension (minimp3 + stb_vorbis + the analysis
front-end).

Reuses a sibling addon's godot-cpp checkout if one exists, otherwise
...
def godot_cpp_dir(branch: str) -> Path:
    ...
    if not (LOCAL_CPP / "SConstruct").exists():
        for name in SIBLINGS:
            shared = HERE.parent / name / "godot-cpp"
            if (shared / ".git").exists():
                print(f"copying godot-cpp from addons/{name} ...")
                subprocess.check_call(
                    ["git", "clone", "--depth", "1", str(shared), str(LOCAL_CPP)])
                break
        else:
...
def main():
    ...
    tmpl = HERE / "song_dsp.gdextension.disabled"
    active = HERE / "song_dsp.gdextension"
