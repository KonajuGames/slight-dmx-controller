...
- `fixture_library.gd` — the `Library` autoload: the bundled fixture
  library (~640 fixtures from Open Fixture Library in `res://fixtures/`).
  A small always-resident catalogue + search / filter; full
  `FixtureProfile`s load on demand from per-manufacturer shards.
  `tools/build_fixture_library.gd` regenerates it from an OFL checkout.
...
**Library...** browses the bundled fixture library — ~640 real fixtures
converted from the [Open Fixture Library](https://open-fixture-library.org)
project (`res://fixtures/`, regenerated by
`tools/build_fixture_library.gd`; see `fixtures/ATTRIBUTION.md`). Search by
manufacturer or model, filter by type / channel count / whether it has
pan-tilt, and preview any fixture's modes and channel layout. **Add to
Patch** drops the chosen profile into the picker for the current session
and selects it — it travels inside the show file when you save, so there's
no copy in `user://` unless you **Edit...** it. Only the small catalogue
index is held in memory; a fixture's full definition loads on demand.
...
- **Fixture profiles**: already implemented — `FixtureProfile` carries
  ...
  regenerates them). Room to grow: an online GDTF-Share / OFL browser that
  fetches definitions on demand through the same catalogue.
